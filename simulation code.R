...
library(xtable)
library(tidyverse)
results<-read.table("500binaryU.txt")
mean(as.numeric(results$V5[-1])- as.numeric(results$V3[-1]))
colnames(results) <- results[1,]
results <-results[-1,-c(1)]
...
tab <- NULL
tab <- rbind(tab,tablerow(estimator=c('MSM withU'), pevalues=results[,2], vevalues=results[,3], truevalue=results[,1]))
tab <- rbind(tab,tablerow(estimator= 'MSM noU', pevalues=results[,4],vevalues=results[,5], truevalue=results[,1]))
tab <- rbind(tab, tablerow('Sensitivity function with correct specification', pevalues=results[,6],vevalues=results[,7], truevalue=results[,1]))
tab <- rbind(tab, tablerow('Sensitivity function with total variance', pevalues=results[,8],vevalues=results[,9], truevalue=results[,1]))
tab <- rbind(tab, tablerow('Sensitivity function without empirical variance', pevalues=results[,8],vevalues=results[,10], truevalue=results[,1]))
tab <- rbind(tab, tablerow('Full Bayes with uniform prior for bias parameters', pevalues=results[,11],vevalues=results[,12], truevalue=results[,1],
                           cilvalues=results[,13], ciuvalues=results[,14]))
# tab <- rbind(tab, tablerow('Full Bayes with general prior', pevalues=results[,14],vevalues=results[,15], truevalue=results[,1]))
for (i in 1:ncol(tab))
  if (is.numeric(tab[,i]))
    tab[,i] <- round(tab[,i], 5)
print(tab)

# Estimator      Mean        RB      SD      SE     SERB CP
# 1                                         MSM withU -10.24321   0.26085 0.43632 0.50658 16.10117 98
# 2                                           MSM noU -11.44132 -11.40528 0.40910 0.44674  9.20113 26
# 3   Sensitivity function with correct specification -10.56476  -2.87013 0.35191 0.45013 27.91026 95
# 4          Sensitivity function with total variance        NA        NA      NA      NA       NA NA
# 5   Sensitivity function without empirical variance        NA        NA      NA      NA       NA NA
# 6 Full Bayes with uniform prior for bias parameters        NA        NA      NA      NA       NA NA
